...
function setup_session_var(user_type, user_info, session) {
    ...
    session.user = {}; 
    session.user.id = user_info.id; //aggiungo attributo id a user
...
function myRedirect(session,res){
    if(!session.user.first_access){
        res.redirect("/auth_router/change_pwd");
        return;
    }
    else{
        res.redirect("/" + session.user.user_type + "/" + session.user.user_type + "_home");
        return;
    }   
}
